...
public class TCP2_ShaderGenerator : EditorWindow
{
	...
	private void FeaturesGUI()
	{
		...
		//Normal Map Blend
		GUISingleFeature("VCOLORS_BUMP_BLENDING", "Normal Map Blending", "Enables 2-way texture blending for the normal map as well", HasFeat("VCOLORS_BLENDING"), true);
		Space();
		//----------------------------------------------------------------
		//Emission
		GUIMask("Emission Map", null, "EMISSION_MASK", "EMISSION_MASK_CHANNEL", "EMISSION", helpTopic:"Self-Illumination Map");
		//Emission Color
		GUISingleFeature("EMISSION_COLOR", "Emission Color", helpTopic:"Self-Illumination Map");
		GUISingleFeature("EMISSION_COLOR_HDR", "HDR Color", "Makes the Emission Color an HDR color that can go outside the [0:1] range (useful for effects like bloom)", HasFeat("EMISSION_COLOR"), true);
		Space();
		//----------------------------------------------------------------
		//Bump
...
//		Space();
		...
		GUISingleFeature("SUBSURFACE_COLOR", "Subsurface Color", "Control the color of the subsurface effect (only affecting back lighting)", HasFeat("SUBSURFACE_SCATTERING"), true, showHelp:false);
		GUISingleFeature("SUBSURFACE_AMB_COLOR", "Subsurface Ambient Color", "Adds an ambient subsurface color, affecting both front and back lighting", HasFeat("SUBSURFACE_SCATTERING"), true, showHelp:false);
		GUISingleFeature("SS_MULTIPLICATIVE", "Multiplicative", "Makes the subsurface scattering effect multiplied to the diffuse color instead of added with it", HasFeat("SUBSURFACE_SCATTERING"), true, showHelp:false);
		Space();
		//----------------------------------------------------------------
		//Cubemap Ambient
		...
		//                 HasFeatOr("SKETCH","SKETCH_GRADIENT"), true);
		//No Obj Offset
		GUISingleFeature("NO_SKETCH_OFFSET", "Disable Obj-Space Offset", "Prevent the screen-space UVs from being offset based on the object's position",
						 HasFeatOr("SKETCH", "SKETCH_GRADIENT"), true);
		Space();
		//----------------------------------------------------------------
		//Outline
		...
		//----------------------------------------------------------------
		//GPU Instancing
		//GUISingleFeature("GPU_INSTANCING", "GPU Instancing Support", "Will enable instancing support if enabled", showHelp: false, indentSpacing: 24f);
		//----------------------------------------------------------------
		TCP2_GUI.SubHeaderGray("Transparency");
		//Alpha Blending
		GUISingleFeature("ALPHA", "Alpha Blending");
		GUISingleFeature("DITHERED_SHADOWS", "Dithered Shadows", "Enables dithered shadows for transparent materials", HasFeat("ALPHA"), true);
		//Alpha Testing
