Shader "Hidden/NTSCShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#define P 1.0
			#define CCFrequency 3.59754545
			#define NotchHalfWidth 2.0
			#define ScanTime 52.6
			#define MaxC 2.1183
			...
			float4 CompositeSample(float2 UV) 
			{
				...
				float4 T = A * Cy * _ScreenParams.x * 2.0 + B + Cx;
			...
			float4 NTSCCodec(float2 UV)
			{
				...
				for(float n = -41.0; n < 42.0; n += 4.0)
				{
					...
					float4 WT = W * (CoordX  + A * CoordY * 2.0 * _ScreenParams.x + B);
