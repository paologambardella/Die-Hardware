Shader "Hidden/TVDistortionShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _yScanline;
			float _xScanline;
			float _vShiftStrength;

			float noise(float2 p)
			{
				float sample = tex2D(_NoiseTex, 
					65.0*float2(1.0,2.0*cos(4.0*_Time.x))
					+ float2(p.x, p.y*0.25)).x;
				sample *= sample;
				return sample;
			}
			...
			float3 getVideo(float2 uv)
			{
				...
				look.y = fmod(look.y + vShift * _vShiftStrength, 1.);

				look = lerp(olduv, look, _distortionStrength);

				float3 video = tex2D(_MainTex, look).xyz;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float vigAmt = 0;//_vignetteStrength*(3.+.3*sin(_Time + 5.*cos(_Time*5.)));
